...
def make_table(schema: str = Query(..., description='The name of the SDSS flag',
                                   examples=['MANGA_DRP2QUAL']), masks: np.recarray = Depends(read_maskbits)):
    ...
    ----------
    schema : str, optional
        _description_, by default Query(..., description='The name of the SDSS flag', examples=['MANGA_DRP2QUAL'])
...
class Maskbits(Base):
    ...
    @router.get("/bits/value", summary='Convert a list of bits into a maskbit value',
                response_model=MaskBitResponse, response_model_exclude_unset=True)
    async def bits_to_value(self, bits: Union[List[int], None] = Query([], description='A list of integer bits', examples=[[2, 8]])) -> dict:
    ...
    @router.get("/bits/labels", summary='Convert a list of bits into their labels',
                response_model=MaskBitResponse, response_model_exclude_unset=True)
    async def bits_to_labels(self, bits: Union[List[int], None] = Query([], description='A list of integer bits', examples=[[2, 8]]),
    ...
    @router.get("/labels/value", summary='Convert a list of labels into a maskbit value',
                response_model=MaskBitResponse, response_model_exclude_unset=True)
    async def labels_to_value(self, labels: Union[List[str], None] = Query([], description='A list of mask labels', examples=[['BADIFU', 'SCATFAIL']]),
                              tab: Table = Depends(make_table)) -> dict:
    ...
    @router.get("/labels/bits", summary='Convert a list of labels into their bits',
                response_model=MaskBitResponse, response_model_exclude_unset=True)
    async def labels_to_bits(self, labels: Union[List[str], None] = Query([], description='A list of mask labels', examples=[['BADIFU', 'SCATFAIL']]),
                             tab: Table = Depends(make_table)) -> dict:
    ...
    @router.get("/value/bits", summary='Decompose a maskbit value into a list of bits',
                response_model=MaskBitResponse, response_model_exclude_unset=True)
    async def value_to_bits(self, value: int = Query(..., description='A maskbit value', examples=[260]), tab: Table = Depends(make_table)) -> dict:
    ...
    @router.get("/value/labels", summary='Decompose a maskbit value into a list of labels',
                response_model=MaskBitResponse, response_model_exclude_unset=True)
    async def value_to_labels(self, value: int = Query(..., description='A maskbit value', examples=[260]), tab: Table = Depends(make_table)) -> dict:
